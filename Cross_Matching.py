...
def convert_to_list(mapping_elements):
    for element, dit in mapping_elements.items():
        for key, apts in dit.items():               #key is ID fot Technique
            dit[key] = list(apts)
    return mapping_elements

def get_sub_technique(df,idx):        #for Titles of IDs
    df = df.copy()
...
def generate_report(source_file, database_file, output_file):    
    ...
    # loading the target source file ( from log analysis )
    ...
    for sheet in database_sheets_list:      #sheet is the name of APT 
        try:
            ...
            # taking insersection of two list , output will be IDs of Techniques
...
def save_in_file(file_name, mapping_dictionary):
    ...
    file.write("ID, " +"Technique Title," +"APTs containig ID\n")
